...
## Start
def birth_death_all_logistic(N, t, a, b, n, K):
    dNdt = N * ( a * N / (N+n) - b - K * (N + n) )
    return dNdt

def birth_death_sterile_logistic(N, t, a, b, n, K):
    dndt = -b * n - K * (n + N) * n
    return dndt

def custom_logistic(N, t, a, g, n, K):
    dndt = -g * N - K * (n + N) * n
    return dndt

def sterile_logistical(y, t, a, b, k):
    n, N = y
    aux_kargs = {'N':N, 't':t, 'a':a, 'b':b, 'n':n, 'k':k}
    dydt = [
        birth_death_sterile_logistic(**aux_kargs),
        birth_death_all_logistic(**aux_kargs)
        ]
    return dydt
...
ax.text(100, pseudo_capacidad/1 + 0.01, f'y={pseudo_capacidad/1:.1E}')

ax.axhline(pseudo_capacidad/4)
ax.text(-25, pseudo_capacidad/4 - 0.02, f'y={pseudo_capacidad/4:.1E}')
